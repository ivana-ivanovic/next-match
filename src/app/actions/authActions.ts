...
export async function signInUser(data: LoginSchema): Promise<ActionResult<string>>{
  try {
    ...
    if(!existingUser || !existingUser.email) return {status: 'error', error: 'Invalid cerdentials'}
    ...
    if (!existingUser.emailVerified) {
      ...
      return {status: 'error', error: 'Please verify your email addres before logging in'}
    }
...
export async function verifyEmail(token: string): Promise<ActionResult<string>> {
  try {
    ...
    if (hasExpired) {
      return {status: "error", error: "Token has expire"}
    }
