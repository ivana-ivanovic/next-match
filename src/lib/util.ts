...
export function handleFormServerErrors<TFiledValues extends FieldValues>(
  errorResponse: {error: string | ZodIssue[]},
  setError: UseFormSetError<TFiledValues>
){
  if(Array.isArray(errorResponse.error)) {
    errorResponse.error.forEach(e => {
    const fieldName = e.path.join(".") as Path<TFiledValues>;
